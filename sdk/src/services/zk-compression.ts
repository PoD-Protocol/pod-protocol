  ...
  async batchSyncMessages(
    ...
    try {
      if (messageHashes.length > 100) {
        throw new ValidationError('Batch size too large. Maximum 100 messages per batch.');
      }
    ...
    try {
      ...
      if (!response.ok) {
        throw new AppError(
          ...
          500,
          `Indexer query failed: ${response.statusText}`,
        );
      }
    ...
    try {
      ...
      if (!response.ok) {
        throw new AppError(
          'INDEXER_QUERY_FAILED',
          500,
          `Stats query failed: ${response.statusText}`,
        );
      }
